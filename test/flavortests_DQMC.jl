...
@testset "Global Updates" begin
    ...
    # Check update probability & stack after accepting
    m = HubbardModelAttractive(2, 6)
    dqmc = DQMC(m, beta = 3.0, safe_mult = 5)

    for waa in 1:10
        conf1 = rand(DQMC, m, dqmc.parameters.slices)
        conf2 = rand(DQMC, m, dqmc.parameters.slices)

        copyto!(dqmc.conf, conf1)
        MonteCarlo.reverse_build_stack(dqmc, dqmc.stack)

        p = 1.0
        for slice in 1:dqmc.parameters.slices
            MonteCarlo.propagate(dqmc)
            @assert MonteCarlo.current_slice(dqmc) == slice
            for i in 1:length(lattice(dqmc))
                if dqmc.conf[i, slice] != conf2[i, slice]
                    r, ΔE, x = MonteCarlo.propose_local(dqmc, m, i, slice, dqmc.conf)
                    p *= real(exp(- ΔE) * r)
                    MonteCarlo.accept_local!(dqmc, m, i, slice, dqmc.conf, r, ΔE, x)
                end
            end
        end
        @assert dqmc.conf == conf2
        MonteCarlo.reverse_build_stack(dqmc, dqmc.stack)
        MonteCarlo.propagate(dqmc)
        temp_stack = deepcopy(dqmc.stack)

        copyto!(dqmc.conf, conf1)
        MonteCarlo.reverse_build_stack(dqmc, dqmc.stack)
        MonteCarlo.propagate(dqmc)

        r, ΔE, x = MonteCarlo.propose_global_from_conf(dqmc, m, conf2)
        @test p ≈ exp(- ΔE) * r 

        MonteCarlo.accept_global!(dqmc, m, conf2, x)
        @test dqmc.conf == conf2
        for field in (:u_stack, :d_stack, :t_stack, :greens, :Ul, :Dl, :Tl, :Ur, :Dr, :Tr)
            @test getfield(temp_stack, field) ≈ getfield(dqmc.stack, field)
        end
    end
end