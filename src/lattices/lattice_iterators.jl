...
function (::EachSiteAndFlavor)(mc::MonteCarloFlavor, model::Model)
    RangeIterator(1 : length(lattice(mc)) * nflavors(model))
end
...
function Base.iterate(iter::_EachLocalQuadByDistance, state)
    ...
    # println("$midx1 $midx2 $fidx1 $fidx2")
    # Branchless version of the above
    # I hope this is actually worth it lol
    # Nm = length(iter.src_mask)
    # src1 = iter.src_mask[midx1]
    # src2 = iter.src_mask[midx2]
    # Nf1 = length(iter.filtered_src2dirtrg[src1])
    # Nf2 = length(iter.filtered_src2dirtrg[src2])

    # fidx2 = mod1(fidx2 + 1, Nf2)
    # d = div(fidx2, Nf2)
    # fidx1 = mod1(fidx1 + d, Nf1)
    # d *= div(fidx1, Nf1)
    # midx2 = mod1(midx2 + d, Nm)
    # d *= div(midx2, Nm)
    # midx1 = midx1 + d

    # midx1 > Nm && return nothing
...
# struct EachLocalQuadBySyncedDistance{T} <: AbstractLatticeIteratorTemplate
#     directions::T
...
# # function (config::EachLocalQuadByDistance)(mc::MonteCarloFlavor, model::Model)
# function (config::EachLocalQuadBySyncedDistance)(lattice::AbstractLattice)
#     key = FilteredSrc2IdxTrg(config.directions)
#     push!(lattice_iterator_cache, key, lattice)
#     push!(lattice_iterator_cache, SrcTrg2Dir(), lattice)
    
#     Ndir = length(lattice_iterator_cache[Dir2SrcTrg()])
#     srctrg2dir = lattice_iterator_cache[SrcTrg2Dir()]
#     filtered_src2dirtrg = lattice_iterator_cache[key]
    
#     src_mask = [i for i in eachindex(filtered_src2dirtrg) if !isempty(filtered_src2dirtrg[i])]
#     N = mapreduce(length, +, filtered_src2dirtrg)
    
#     # Total length: every site * sum(every valid (dir, trg) for every src site)
#     # This is probably wrong in some cases, so let's hard calculate it

#     count = 0
#     midx1 = midx2 = fidx1 = 1; fidx2 = 0
#     while true
#         src1 = src_mask[midx1]
#         src2 = src_mask[midx2]
#         # These need to be overwritten
#         dir1 = dir2 = trg1 = trg2 = 0

#         # Branchless V2
#         while true
#             b1 = fidx2 == length(filtered_src2dirtrg[src2])
#             b2 = b1 && (fidx1 == length(filtered_src2dirtrg[src1]))
#             b3 = b2 && (midx2 == length(src_mask))
#             b4 = b3 && (midx1 == length(src_mask))
#             if b4; @goto DONE end
#             fidx2 = Int64(b1 || (fidx2 + 1))
#             fidx1 = Int64(b2 || (fidx1 + b1))
#             midx2 = Int64(b3 || (midx2 + b2))
...
#             src1 = src_mask[midx1]
#             src2 = src_mask[midx2]
#             dir1, trg1 = filtered_src2dirtrg[src1][fidx1]
#             dir2, trg2 = filtered_src2dirtrg[src2][fidx2]
#             (dir1 == dir2) && break
#         end
...
#         (Ndir, Ndir * length(config.directions)), 
#         count, src_mask, srctrg2dir, filtered_src2dirtrg
#     )
# end

# function Base.iterate(iter::_EachLocalQuadBySyncedDistance, state = (1,1,1,0))
#     midx1, midx2, fidx1, fidx2 = state
#     src1 = iter.src_mask[midx1]
#     src2 = iter.src_mask[midx2]
#     # These need to be overwritten
...
#     dir12 = iter.srctrg2dir[src1, src2]
#     combined_dir = dir12 + iter.mult[1] * (dir1-1)
#     # state = (src1 mask index, src2 mask index, filter1 index, filter2 index)
#     return ((combined_dir, src1, trg1, src2, trg2), (midx1, midx2, fidx1, fidx2))
# end


# function ndirections(iter::_EachLocalQuadBySyncedDistance)
#     (iter.mult[1], div(iter.mult[2], iter.mult[1]))
# end
...
# function Base.iterate(iter::EachLocalQuadByDistance, state=1)
...
#     end
# end
function Base.iterate(iter::EachLocalQuadByDistance{K}, state=(1,1)) where {K}
    i, j = state
...
function Base.iterate(iter::EachLocalQuadBySyncedDistance{K}, state = (1, 1)) where {K}
    i, j = state
    if i <= length(iter.implied)
        N = length(iter.implied[i])
...
Requires `lattice` to implement `positions` and `lattice_vectors`.
"""
struct EachLocalQuadByDistance2{K} <: DeferredLatticeIterator
    N::Int64
    implied::Array{Vector{NTuple{4, UInt16}}, 3}
end


function EachLocalQuadByDistance2{K}(lattice::AbstractLattice) where {K}
    # (src1, src2) pairs from dir
    # pairs_by_dir = EachSitePairByDistance(lattice)
    dir2srctrg = lattice_iterator_cache[Dir2SrcTrg()]

    # (dir, trg1) from src1
    trg_from_src = [Tuple{Int64, Int64}[] for _ in 1:length(lattice)]
    for dir in 1:K
        for (src, trg) in dir2srctrg[dir] #in_direction(pairs_by_dir, dir)
            push!(trg_from_src[src], (dir, trg))
        end
    end
    # trg_from_src = lattice_iterator_cache[Src2DirTrg()]
    ...
    # linear = Vector{NTuple{7, UInt16}}(undef, total_length)
    # k = 1
    implied = [NTuple{4, UInt16}[] for _ in 1:length(dir2srctrg), _ in 1:K, _ in 1:K]
    dir12, idx, i, j = (1,1,1,1)
    while true
        if dir12 ≤ length(dir2srctrg)
            if idx ≤ length(dir2srctrg[dir12])
                src1, src2 = dir2srctrg[dir12][idx]
                if i ≤ length(trg_from_src[src1])
                    dir1, trg1 = trg_from_src[src1][i]
                    if j ≤ length(trg_from_src[src2])
                        dir2, trg2 = trg_from_src[src2][j]
    ...
    EachLocalQuadByDistance2{K}(total_length, implied)
end
function EachLocalQuadByDistance2{K}(mc::MonteCarloFlavor, model::Model) where {K}
    EachLocalQuadByDistance2{K}(lattice(model))
end


function Base.iterate(iter::EachLocalQuadByDistance2{K}, state=(1,1)) where {K}
    i, j = state
    if i <= length(iter.implied)
        N = length(iter.implied[i])
...
ndirections(iter::EachLocalQuadByDistance2{K}) where {K} = size(iter.implied)
Base.length(iter::EachLocalQuadByDistance2) = iter.N
Base.eltype(::EachLocalQuadByDistance2) = Tuple{Int64, UInt16, UInt16, UInt16, UInt16}
...
Creates an iterator template which yields 4-tuples of sites with 2 directions
inbetween. The output is given as `(combined_dir, src1, trg1, src2, trg2)`
where `combined_dir` is a linear index of `(dir12, dir)` corresponding to
the distance vectors `(src2 - src1, trg1 - src1 = trg2 - src2)`. 
...
Requires `lattice` to implement `positions` and `lattice_vectors`.
"""
struct EachLocalQuadBySyncedDistance2{K} <: DeferredLatticeIterator
    N::Int64
    implied::Array{Vector{NTuple{4, UInt16}}, 2}
end


function EachLocalQuadBySyncedDistance2{K}(lattice::AbstractLattice) where {K}
    # (src1, src2) pairs from dir
    # pairs_by_dir = EachSitePairByDistance(lattice)
    dir2srctrg = lattice_iterator_cache[Dir2SrcTrg()]
    ...
    implied = [NTuple{4, UInt16}[] for _ in 1:length(dir2srctrg), _ in 1:K]
    dir12, idx, i, j = (1,1,1,1)
    while true
        if dir12 ≤ length(dir2srctrg)
            if idx ≤ length(dir2srctrg[dir12])
    ...
    EachLocalQuadBySyncedDistance2{K}(total_length, implied)
end
function EachLocalQuadBySyncedDistance2{K}(mc::MonteCarloFlavor, model::Model) where {K}
    EachLocalQuadBySyncedDistance2{K}(lattice(model))
end


function Base.iterate(iter::EachLocalQuadBySyncedDistance2{K}, state = (1, 1)) where {K}
    i, j = state
    if i <= length(iter.implied)
        N = length(iter.implied[i])
...
ndirections(iter::EachLocalQuadBySyncedDistance2{K}) where {K} = size(iter.implied)
Base.length(iter::EachLocalQuadBySyncedDistance2) = iter.N
Base.eltype(::EachLocalQuadBySyncedDistance2) = Tuple{Int64, UInt16, UInt16, UInt16, UInt16}
