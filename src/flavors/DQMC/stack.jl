...
function init_hopping_matrices(mc::DQMC, m::Model)
    ...
    if !mc.parameters.checkerboard
        ...
        l = lattice(mc)
        mc.stack.hopping_matrix_exp = CheckerboardDecomposed(T, l, T -> fallback_exp(-0.5 * dtau * T), false)
        mc.stack.hopping_matrix_exp_inv = CheckerboardDecomposed(T, l, T -> fallback_exp(0.5 * dtau * T), false)
        mc.stack.hopping_matrix_exp_squared = CheckerboardDecomposed(T, l, T -> fallback_exp(-0.5 * dtau * T), true)
        mc.stack.hopping_matrix_exp_inv_squared = CheckerboardDecomposed(T, l, T -> fallback_exp(0.5 * dtau * T), false)
    end
